...
//
// Two shapes need real parsing (level rows, statement rows); everything else is a fixed
// literal shape. Whitespace-only differences in *where* a token boundary falls inside a
// run of spaces are visually invisible — PunchCard.astro's renderer recolors every space
// character to `.pcc-empty` regardless of which token it belongs to — so the splitting
// rules below favor simplicity over exactly mirroring hand-authored legacy spacing.
